...
public:
	...
	void addTextW(const wchar_t *displayStr, const char* replyStr);
	void clearText();
	const wchar_t* getTextStringW(unsigned int index);
...
private:
	...
	wchar_t *textString[TEXT_BUFFER_SIZE]; // This is the text buffer alias display string
...
public:
	TextPageScroll(DrawText *drawText, float centerX, float upperY, float width, float height);
...
extern TextPageScroll *menuPage;
