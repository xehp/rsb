...
GLuint drawImageTextureFactory(int xOffset, int yOffset, int width, int height)
{
	...
	size_t s = width*height*bpp; // Size of texture in bytes

	unsigned char *tmp=(unsigned char *)malloc(s); // free(tmp) is to be done later in this method
	assert(tmp!=NULL);
	...
	glBindTexture( GL_TEXTURE_2D, textureId );
		...
		// This works on native but not on web
...
#else
		// This works on both native and on web
