...
#ifndef __EMSCRIPTEN__
void YukigassenBillboard::writeSelf(WordWriter *ww)
{
	YukigassenProperty::writeSelf(ww);

	ww->writeInt(textureIndex);/*rootUpdateCounter*/
	ww->writeInt(entityColor);/*creationTime*/
}
#endif
...
void YukigassenBillboard::setupInternalRenderingData()
{
	...
	const GLfloat z = roomSize.z/2.0f*windowObject->mainAspectRatio();
