...
int hca_client(const char *hostname, int port)
{
  ...
  if (err) 
  {
    printf("hsa: socket %d fcntl returned error code %d\n", s, err);
  }
...
int hca_read(int fd,char *buf, int len)
{
  ...
  if (len<0) /* Fel? */
  {
    ...
    else
    {
      // Something wrong
      perror("read");
      printf("hca_read: socket %d error %d\n",fd,errno);
      return(-2);
    }
  }
