...
void YukigassenTextBillboard::setupInternalRenderingData()
{
	...
	const GLfloat z = height*mainAspectRatio;
	...
	if (texture_id==0)
	{
		...
		texture_id = drawTextTextureFactory("Avatar", 2); // This creates an image with the desired text. Remember to delete texture (glDeleteTextures) when no longer needed.
	}
...
void YukigassenTextBillboard::setText(const char *str)
{
	...
	if ((str!=NULL) && (*str))
	{
		texture_id = drawTextTextureFactory(str, len); // This creates an image with the desired text. Remember to delete texture (glDeleteTextures) when no longer needed.
	}

	width = len * height / (2 * mainAspectRatio);
