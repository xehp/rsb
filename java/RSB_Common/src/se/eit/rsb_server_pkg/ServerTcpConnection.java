...
public class ServerTcpConnection {
	...
	public ServerTcpConnection(D1SubRoot db, WebConnection cc)
	{
		this.db=db;
		this.cc=cc;
	}
	...
	// TODO Move this to D1SubRoot
	public D1SubRoot findOrCreateGameDb()
	{		
		D1SubRoot r = (D1SubRoot) db.findOrCreateChildObject(WorldBase.nameOfWorldsDb, WorldBase.typeOfWorldsDb);
		
		// TODO Use findOrCreateChildObject here also: D1SubRoot g = r.findOrCreateChildObject(gameTypeName, D1SubRoot.className());
		D1SubRoot g = r.findDb(gameTypeName);
		if (g==null)
		{
			g = new D1SubRoot();
			r.lockWrite();
	...
	public D1SubRoot findPlayersDb()
	{
		return db.findDb(UserAccount.nameOfPlayersDb);
	}
	
	
	public D1SubRoot findOrCreatePlayersDb()
	{
		return (D1SubRoot)db.findOrCreateChildObject(UserAccount.nameOfPlayersDb, UserAccount.typeOfPlayersDb);
	}
	...
	{		
    	...
    	D1SubRoot gamesDatabase = findOrCreateGameDb();
		...
		D1SubRoot ro = gamesDatabase.findDb(name);
    ...
    */
	...
	{	
		...
		while(++n<100)
		{
			...
			else
			{
				...
				debug("Answer was not a string, answer='"+str+"', tag="+tag); // it did not begin with double quotes
			}
		}
		debug("to many tries");
		return null;
    ...
    //  the number of the chosen alternative, zero counting
	//  -1 for timeout, cancel or error
	public int promptButtons(String tag, String defaultText, D1List<String> buttonNames)
	{	
		String names="";
		int n=0;
	...
	*/
	...
	public void alertBox(String tag, String defaultText)
	{
		if (dontUseRef)
		{
			...
			D1List<String> optionsList=new D1List<String>();
			optionsList.add("OK");
    	...
    	while (worldName==null)
    	{
    		...
    		else 
    		{
	    		...
	    		{ 
	    			...
	    			D1Named go = findWorld(worldName);
	    	    	if (go==null)
	    	    	{
	    	    		// The name is free	    	    		
