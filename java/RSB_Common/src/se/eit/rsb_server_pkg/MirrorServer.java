...
import se.eit.d1_pkg.D1NotificationReceiver;
...
public abstract class MirrorServer extends ServerBase implements D1NotificationReceiver {

	
	public static String className()
	{	
		// http://stackoverflow.com/questions/936684/getting-the-class-name-from-a-static-method-in-java		
		return MirrorServer.class.getSimpleName();	
	}	
	...
	protected D1ThreadSafe playerAvatar; // TODO: This shall be of some avatar base class or use AvatarPlayerReferences
	protected GameUser gameUser; // TODO: This shall be of some avatar base class or use AvatarPlayerReferences
	...
	*/
	...
	protected void sendUpdateAndRemember(D1Storable io) throws IOException
	{
		...
		// TODO in future we may have more dependencies
		final D1Container parentObject=io.getParent();
		if (parentObject!=null)
		{
			...
			if (parentId>=0)
			{
				...
				if (parentCounter==null)
				{
					...
					sendUpdateAndRemember((D1IdObj)parentObject);
				}
				else
				{
		...
		ww.writeWord((sentCounter==null) ? "ma" : "mu"); // "mirrorAdd" : "mirrorUpdate"
	...
	protected int sendUpdateAndRememberIfVisible(D1Storable io) throws IOException
	{
	...
	public int findRemovalsAndRemember() throws IOException, InterruptedException
	{
		...
		while(iterator.hasNext())
		{
			...
			final D1IdObj io=worldBase.getDbIdObj(key);
	...
	public int findAndSendDbUpdatesToClientAll() throws IOException, InterruptedException
	{
		...
		try
		{
			...
			for (final D1IdObj io : worldBase.idList.dbListId)
			{
	...
	*/
	...
	public int updateObjToClient(final D1Storable io) throws IOException
	{
		{
			final int id = io.getId();
			final Integer prevChangeCounter=latestVersionOfIdObjectSent.get(id);
			
			// Is the object known to client?
	...
	public int updateObjToClientRecursive(final D1IdObj io) throws IOException
	{
		...
		{
			for(D1Storable db: io.listOfStoredObjects)
			{
				n+=updateObjToClientRecursive((D1IdObj)db);
			}
		}
		    ...
		    try 
		    {
			    try 
			    {
			    	for(;;) // loop until InterruptedException
			    	{
						...
						final D1IdObj io=worldBase.getDbIdObj(id);
	...
	public void sendMirrorUpdated() throws IOException
    {
	...
	public void notifyRef(int subscribersRef, int sendersRef)
	{
		...
		{				
			final D1Base db = worldBase.getDbIdObj(sendersRef);
			if (db!=null)
			{
	...
	{
		// Remember to always call removeNotificationReceiver when leaving. 
		notifyIdx=worldBase.addNotificationReceiver(this, 0); 
	}
	...
	public void close()
	{
		...
		if (notifyIdx!=-1)
		{
			worldBase.removeNotificationReceiver(notifyIdx);
			notifyIdx=-1;
		}
