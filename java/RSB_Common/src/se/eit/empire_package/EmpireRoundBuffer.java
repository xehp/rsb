...
public class EmpireRoundBuffer  extends EmpireBase {
	...
	int maxObjects=64; // max number of messages stored in the round buffer, max index is: maxObjects*2-1
    int head=0; // This is the first (oldest) message in the queue
    int tail=0; // This is where messages are added to the queue
	int msgCount=0;

	public EmpireRoundBuffer(D1Base parent, String name) 
	{
		super();
	...
	public void postMessageToThis(String str)
	{
		...
		for(int i=0;i<this.getMaxIndex();i++)
		{
			...
			{
				D1Base d=this.getObjFromIndex(head);
				if (d!=null)
				{
					this.deleteObject(d);
				}
		...
		{
			D1Base d=this.getObjFromIndex(tail);
			if (d!=null)
			{
