...
public abstract class WorldBase extends D1NotificationSender
{
	
	public final static String nameOfWorldsDb = "worldsDatabase"; // worldsDatabase
    public final static String typeOfWorldsDb = "D1NoSaveRoot"; 
    ...
    public Random generator = new Random(); // deprecated, use random
	...
	public WorldBase(D1Container parent) 
	{
		super();

		D1SubRoot ro = parent.getDbSubRoot();
		ro.lockWrite();
		try
		{
	...
	*/
	...
	// For FPS games the unit here shall be milliseconds (ms) but for strategy games it is just unspecified time units.
	...
	*/
	...
	// All game worlds must override this so it shall be abstract.
	public abstract void generateSelf();


	// This returns the name of the server object that clients shall use to play this world.
	// To those implementing this in an extending class: The name used needs to match that in serverFactory	
