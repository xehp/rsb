// UserAccount.java, was: PlayerData.java
//
...
import se.eit.rsb_package.PasswordStorage.CannotPerformOperationException;
import se.eit.rsb_package.PasswordStorage.InvalidHashException;
import se.eit.web_package.*;




public class UserAccount extends D1TickReceiver {

    public final static String nameOfPlayersDb = "playersDatabase"; 
    public final static String typeOfPlayersDb = "D1NoSaveRoot"; 
    
    private String password=null; // TODO: Do not store password in clear text!
    ...
    public int playerCode=0; // Used when moving between servers 
    static Random generator = new Random();

    protected String tmpPassword=null;
    ...
    public UserAccount(D1Container parent, String name, String registeredFromAdr, String emailAddress, D1GlobalConfig globalConfig) {
        super();
        if (parent!=null)
        {
...
/*
    ...
    */
	...
	final static public char specRight=39; // should be 37?
	final static public char specUp=38;
	final static public char specLeft=37; // should be 39?
	final static public char specDown=40;
	final static public char specDel=46;
    ...
    */
	...
	public void listPlayerPreferences(WordWriter ww) throws IOException
	{
		...
		ww.writeLine("Keys that are mapped to a client function:");
		ww.incIndentation();
    	for(int i=0; i<actionForKey.length;i++)
    	{
    		int s=actionForKey[i];
		...
		ww.writeLine("Keys that activate a custom command:"); 	
		ww.incIndentation();
    	for (Integer key: quickKeyCmdHashMap.keySet())
    	{
    		String value=quickKeyCmdHashMap.get(key);	    		
...
*/
	...
	public boolean setPassword(String pw)
	{
		if (pw!=null)
		{
			try {
				...
				password=PasswordStorage.createHash(pw); 
		...
		{
			// No password, null means client is locked out.
			password=null;
		}
	...
	public boolean isPasswordOk(String pw)
	{
		if (password==null)
		{
			debug("isPasswordOk: Player "+this.getNameAndId()+" is not allowed to log in.");			
			return false;			
		}
		if ((tmpPassword!=null) && (tmpPassword.equals(pw)))
		{
			tmpPassword=null;
			return true;		
		}
		try {
			if (PasswordStorage.verifyPassword(pw, password))
			{
				tmpPassword=null;
				return true;
			}
