...
public class WordWriter {
	...
	{		
		...
		final String allowedCharacters=" !$%'()*+,-./:;<=>?@[]_^|~";
	...
	{
		System.out.println(safeStr(": "+str, 520));
	}
	...
	public void writeLine(String line)
	{
		writeIndentation();
		//tmpBuf.append(line);
		copyAndAddEscSequence(line, tmpBuf);
		writeEoln();
	}	
						...
						if ((ch<0) || (ch>=127))
						{
							if (ch>=256)
							{
								if (ch>=0x10000)
								{
									sb.append('\\');
									sb.append('U');							
									sb.append((char)encodeHex((ch>>28)&0xf));
									sb.append((char)encodeHex((ch>>24)&0xf));
									sb.append((char)encodeHex((ch>>20)&0xf));
									sb.append((char)encodeHex((ch>>16)&0xf));
									sb.append((char)encodeHex((ch>>12)&0xf));
									sb.append((char)encodeHex((ch>>8)&0xf));
									sb.append((char)encodeHex((ch>>4)&0xf));
									sb.append((char)encodeHex(ch&0xf));								
								}
								else
								{
									sb.append('\\');
	...
	*/
	...
	public static int encodeHex(int c)
	{
		if (c<10)
		{
			return '0'+c;
