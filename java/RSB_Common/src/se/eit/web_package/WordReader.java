...
public class WordReader {
	...
	*/
	...
	public byte readByte() throws NumberFormatException
	{
		...
		{
			WordWriter.safeError("\nWordReader.readInt: NumberFormatException expected byte but found '" + str + "', inputStr='"+inputStr+"'");
	...
	public int readInt() throws NumberFormatException
	{
		...
		{
			WordWriter.safeError("WordReader.readInt: NumberFormatException expected int but found '" + "', inputStr='"+inputStr+"'");
			throw(new NumberFormatException("expected int but found '" + str + "', inputStr='"+inputStr+"'"));
	...
	public long readLong() throws NumberFormatException
	{
		...
		{
			WordWriter.safeError("\nWordReader.readInt: NumberFormatException expected long but found '" + str + "', inputStr='"+inputStr+"'");
	...
	public float readFloat() throws NumberFormatException
	{
		...
		{
			WordWriter.safeError("\nWordReader.readFloat: NumberFormatException expected float but found '" + str + "', inputStr='"+inputStr+"'");
...
*/
	...
	// Inside a number (not first character) some additional characters are allowed.
	...
	public static boolean singleCharTokens(char ch)
	{
		switch(ch)
		{
		// ,;[](){}~
	...
	public static boolean isMultiCharToken(String str, char ch)
	{
		if (str.length()==0)
		{
			switch(ch)
		...
		while (state!=ReadTokenState.EndState)
		{
			...
			switch(state)
			{
				case InitialState: // initial state, skipping spaces and separators
				{
					...
					{
						// This was a leading space or a separator char
						// skip leading spaces
						// TODO any number of spaces/blanks are allowed but we should not allow any number of other separators such as ','.
						getWaitChar();
					}
					...
					else if (singleCharTokens(ch))
					{
						sb.append(getWaitChar());
				 		state=ReadTokenState.EndState;			 			
					}
					else if (isMultiCharToken(sb.toString(),ch))
					{
						sb.append(getWaitChar());
				...
				{
					if (isMultiCharToken(sb.toString(),ch))
					{
						sb.append(getWaitChar());
					}
					else
					...
					default:
						{
							if ((ch>='0') && (ch<='9'))
							{
								compositeChar=0;
								stateCounter=3;
								state=ReadTokenState.ParseOctal;
							}
							else
							{
								debug("unknown escape sequence ignored");
								sb.append('\\');
								sb.append(ch);
								break;
							}
				...
				case ParseHex:
					compositeChar=(compositeChar<<4)+decodeHex(getWaitChar());
					--stateCounter;
					if (stateCounter<=0)
					{
						if ((compositeChar<0) || (compositeChar>=0x1000))
						{
							// Java use UTF16 which creates more headaches by using Surrogate pairs, so keep these as esc sequences
							if (compositeChar<0x10000)
							{
								sb.append('\\');
								sb.append('u');
								sb.append((char)WordWriter.encodeHex((compositeChar>>12)&0xF));
								sb.append((char)WordWriter.encodeHex((compositeChar>>8)&0xF));
								sb.append((char)WordWriter.encodeHex((compositeChar>>4)&0xF));
								sb.append((char)WordWriter.encodeHex((compositeChar>>0)&0xF));			
							}
							sb.append('\\');
							sb.append('U');
							sb.append((char)WordWriter.encodeHex((compositeChar>>28)&0xF));
							sb.append((char)WordWriter.encodeHex((compositeChar>>24)&0xF));
							sb.append((char)WordWriter.encodeHex((compositeChar>>20)&0xF));
							sb.append((char)WordWriter.encodeHex((compositeChar>>16)&0xF));
							sb.append((char)WordWriter.encodeHex((compositeChar>>12)&0xF));
							sb.append((char)WordWriter.encodeHex((compositeChar>>8)&0xF));
							sb.append((char)WordWriter.encodeHex((compositeChar>>4)&0xF));
							sb.append((char)WordWriter.encodeHex((compositeChar>>0)&0xF));
						...
						{
							sb.append((char)compositeChar);
						}
						state=ReadTokenState.InsideString;
					}
					break;
				case ParseOctal:
					compositeChar=(compositeChar<<3)+decodeHex(getWaitChar());
					--stateCounter;
					if (stateCounter<=0)
					{
						sb.append((char)compositeChar);
	...
	static public int decodeHex(int ch)
	{
		if ((ch>='0') && (ch<='9'))
		{
			return ch-'0';
